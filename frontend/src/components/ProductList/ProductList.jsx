...
export default function ProductList() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await productApi.getProductsBySearch(currentPage - 1, resultsPerPage, searchQuery);
    ...
    fetchData();
  }, [currentPage, searchQuery, resultsPerPage]); // Add resultsPerPage as a dependency
        ...
        <Grid container direction={'row'} spacing={0.1} wrap="noWrap">
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={9}>
              {searchQuery && (
                <Typography variant="h5" sx={{ mb: 3, fontWeight: 700 }} id="search-results-header">
